...
def test_targets():
    ...
    mcl1_dict = {'date': datetime.date(2019, 12, 13), 'dir': '2019-12-13_mcl1', 'name': 'mcl1'}
    ...
    test_target ='aaa'
    with pytest.raises(ValueError, match=f'Directory for target {test_target} not found.'):
...
def test_target_class():
    ...
    columns = [ 0, 1, 'Smiles1', 'Smiles2', 'exp. DeltaG [kcal/mol]',  'exp. Error [kcal/mol]']
    pd.testing.assert_frame_equal(tgt.getEdgeSet().getDF(columns=columns), edgeSet.getDF(columns=columns))
...
def test_targetSet():
    ...
    with pytest.raises(ValueError, match=f"Column xxx is not known and cannot be generated."):
